...
install_app_to_bench() {
    ...
    ynh_exec_warn_less ynh_exec_as $app $install_dir/dokos-bench-folder/env/bin/activate
    ynh_exec_warn_less ynh_exec_as $app $install_dir/dokos-bench-folder/env/bin/python -m pip install --upgrade -e $apps_dir/$1

    chown -R $app: "$apps_dir/$1"

    local apps_list=$install_dir/dokos-bench-folder/sites/apps.txt
    if test -f "$apps_list"; then
        ynh_exec_warn_less ynh_exec_as $app echo -e "\n$1" >> $apps_list
    else 
        ynh_exec_warn_less ynh_exec_as $app echo "$1" >> $apps_list
    fi

    pushd $apps_dir/$1
        ynh_use_nodejs
        ynh_exec_warn_less ynh_exec_as $app env $ynh_node_load_PATH yarn install --check-files

        # Builds assets for the Frappe Applications installed on bench
        ynh_exec_warn_less ynh_exec_as $app env PATH=$install_dir/bin:$PATH bench build --app $1
    popd
